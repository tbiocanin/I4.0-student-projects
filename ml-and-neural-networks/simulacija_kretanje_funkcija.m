function [x_translatorno, y_translatorno, teta_izlaz, delta_s_levo_izlaz, delta_s_desno_izlaz] = simulacija_kretanje_funkcija(x_pocetno)
    ...
    %b = 200;
    %delta_s_levo = 10 + randn(1, 100) %debagovati ovaj deo jer samo mogu translatornu trajektoriju da dobijem ovde, dakle nalik kretanju za slovo. Proveri formulu
    %delta_s_desno = 10 + randn(1, 100)
    
    b = 400;
    R = 100;
    alfa = -2*pi/180;
    delta_s_levo = ones(1, 10) .* (R - b/2)*alfa + randn(1, 10);
    delta_s_desno = ones(1, 10) .* (R + b/2)*alfa + randn(1, 10);
    ...
    for i = 2:10
        ...
        x_novo(2, i) = 2*cos(x_novo(2, i)) + cos(x_novo(2, i)/2 + pi/2) + 10;
